...
def generate_image(topic):
    ...
    similarities = cosine_similarity(encoded_topic, np.transpose(embedded_tweets))
    ...
    sc = ax.scatter(data_tsne[:, 0], data_tsne[:, 1], c=similarities, cmap="viridis", norm=norm)
...
if USE_STANDARD_INTERFACE:
    ...
    minimum_lenght = gr.Number(value=120, default=120, label="Minimum number of characters")
    demo = gr.Interface(
        fn=search,
        inputs=[
            ...
            n_tweet_component,
            minimum_lenght,
            gr.Textbox(lines=1, value="Rock, Metal, EDM, Dance, Heavy Metal, Techno, Funky, Grunge, Pop, Kpop, Rap"),
        ],
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                n_tweet_component =  gr.Number(value=100, default=20, label="Number of Tweets")
                minimum_lenght = gr.Number(value=120, default=120, label="Minimum number of characters")
